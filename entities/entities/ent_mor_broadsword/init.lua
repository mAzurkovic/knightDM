...
function ENT:PhysicsCollide(data, phys)
	...
	elseif Ent.Health and self.HitEnemy == false then	// Only deal damage once.
		...
		Ent:TakeDamage(50, self:GetOwner(), boink)	// If you have a killicon for the broadsword, this makes it appear.
		boink:Remove()
...
function ENT:Use(activator, caller)
	if activator:GetWeapon(self.Weapon) != NULL then return end	// If the player has the broadsword already, don't pick it up.
